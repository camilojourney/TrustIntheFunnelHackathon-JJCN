...
## Prompt 7.5 — Persistent database (course correction)

**Goal:** Replace every in-memory dict from Prompts 3–7 with real persistence so data survives process restarts, using SQLite (via SQLAlchemy + Alembic) — swappable to Postgres later by changing `DATABASE_URL`.

**Why:** All prior prompts stored state in module-level Python dicts (`_applications`, `_claims_by_candidate`, `_interviews`, `_evidence_by_claim`, `_reports`). This is a real gap for anything beyond a single-process demo — restarting the server wiped all candidate/interview/report data.

**What changed:**
- [backend/app/db.py](../backend/app/db.py) — SQLAlchemy `engine`/`SessionLocal`/`Base`, `get_db()` FastAPI dependency. `DATABASE_URL = "sqlite:///./claimproof.db"`.
- [backend/app/models.py](../backend/app/models.py) — ORM tables: `CandidateModel`, `ApplicationModel`, `ClaimModel`, `InterviewModel`, `QuestionModel`, `AnswerModel`, `EvidenceModel`, `AssessmentModel`.
- Every route module (`applications.py`, `interviews.py`, `evidence.py`, `reports.py`) now takes `db: Session = Depends(get_db)` and reads/writes ORM rows instead of dicts. `find_claim()`/`find_role_title()` now take a `db` parameter.
- [backend/app/services/seed.py](../backend/app/services/seed.py) — `reset_demo_state(db)` now upserts `CandidateModel`/`ClaimModel` rows instead of mutating a module dict.
- [backend/app/main.py](../backend/app/main.py) — calls `Base.metadata.create_all(bind=engine)` at import time as a safety net (tables exist even if migrations weren't run), consistent with the project's "deterministic demo fallback" philosophy.
- Alembic scaffolding added: `backend/alembic.ini`, `backend/migrations/` (env.py wired to `app.db.Base.metadata` and `app.models`), with one autogenerated initial migration creating all 8 tables.

**Setup (run once per fresh clone):**
```bash
cd backend
conda activate claimproof-backend   # or your venv
...
curl -X POST localhost:8000/api/applications -d '{"use_seed": true}' -H 'Content-Type: application/json'
# ... run the full interview flow from Prompts 4-7 ...
curl -X POST localhost:8000/api/interviews/<id>/complete
kill %1   # stop the server entirely
uvicorn app.main:app --port 8000 &
curl localhost:8000/api/candidates/demo-candidate-1/report
```
Expect the report to return correctly **after the restart**, proving data is persisted rather than held in memory.

**Note:** `backend/claimproof.db` is gitignored (see `.gitignore`) — it's local demo state, not something to commit. Anyone pulling the repo fresh needs `alembic upgrade head` before their first run.

...
- Live API data for the three additional candidates. They use seeded files;
  requesting live data falls back to the corresponding fixture.
- The static demo PDF. All four candidates use the print route.
- Live sponsor observability. Completed interviews link to the local execution trace.

The ML Engineer queue entry uses the backend's canonical `demo-candidate-1`
ID. The older `/recruiter/demo` URL remains an alias for that candidate,
including live reports and the recruiter's saved next step.

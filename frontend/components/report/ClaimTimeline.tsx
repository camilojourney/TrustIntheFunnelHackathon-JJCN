      ...
      {tab === "questions" && (
        <div role="tabpanel" id={panelId("questions")} aria-labelledby={tabId("questions")}>
          ...
          <ol className={LIST}>
            ...
            {/* One step per exchange. A separate ANSWER step repeated what
                the CANDIDATE ANSWER label in the block already says. */}
            {exchanges.map(({ question, answer }) => (
              ...
              >
                <QuestionBlock kind={question.kind} text={question.text} chip={false} />
                <AnswerBlock text={answer?.transcript} indent={false} recordId={answer?.id} />
              </Step>
            ))}
          </ol>
        </div>

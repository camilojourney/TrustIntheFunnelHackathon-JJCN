import { canonicalCandidateId } from "@/lib/candidates";
...
function seeded(id: string): CandidateReport {
  return SEEDED[canonicalCandidateId(id)] ?? demoReport;
}
...
async function fetchReport(id: string): Promise<CandidateReport | null> {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/candidates/${encodeURIComponent(canonicalCandidateId(id))}/report`, {
      cache: "no-store",
